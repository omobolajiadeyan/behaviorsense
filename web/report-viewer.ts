...
const formatBytes = (value: number): string => new Intl.NumberFormat().format(value);
...
function renderReport(report: BehaviorReport): void {
...
async function loadSample(): Promise<void> {
  ...
  renderReport(await response.json() as BehaviorReport);
}

async function loadFile(file: File): Promise<void> {
  renderReport(JSON.parse(await file.text()) as BehaviorReport);
}
